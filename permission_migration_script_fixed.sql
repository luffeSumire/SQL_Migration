...
-- 建立暫存對應表
IF OBJECT_ID('tempdb..#UrlMapping') IS NOT NULL
...
-- 新增橫幅輪播對應 (根據 permission_map.md 注意事項)
INSERT INTO #UrlMapping (old_url, new_route, rule_name) VALUES
...
SELECT 
    sg.sid,
    CAST(sg.sid AS VARCHAR(100)) as groupCode,
    sg.cname as name,
...
SELECT DISTINCT
    ...
    CASE 
        WHEN p.action = 'C' THEN sgr.c
        WHEN p.action = 'R' THEN sgr.r  
        WHEN p.action = 'U' THEN sgr.u
        WHEN p.action = 'D' THEN sgr.d
        ELSE 0
    END as allow,
    GETDATE() as createTime,
    1 as createUser,
...
WHERE sgr.groups_sid IS NOT NULL
  ...
  AND (
    (p.action = 'C' AND sgr.c IS NOT NULL) OR
    (p.action = 'R' AND sgr.r IS NOT NULL) OR  
    (p.action = 'U' AND sgr.u IS NOT NULL) OR
    (p.action = 'D' AND sgr.d IS NOT NULL)
  )
...
PRINT '=== 帳號權限群組對應清單 ==='  
SELECT accountSid, groupSid FROM account_permission_group ORDER BY accountSid, groupSid
