-- ==========================================
-- 權限系統資料遷移腳本 (修正版)
-- 參考 11_account_migration 模式，避免覆蓋新系統設置
...
-- STEP 1: 清理策略 - 保護新系統設置
-- ==========================================
PRINT '=== 開始清理權限資料 (保護新系統設置) ===';

-- 清理權限群組對應 (但保留系統管理員群組的設置)
DELETE FROM permission_group_map 
WHERE groupSid != 3;  -- 保護系統管理員群組 (sid=3) 的權限設置
PRINT '已清理 permission_group_map 表 (保護系統管理員群組)';

-- 清理帳號權限群組對應 (但保護系統管理員 AccountId=1)
DELETE FROM account_permission_group 
WHERE accountSid != 1;  -- 保護系統管理員帳號 (AccountId=1)
PRINT '已清理 account_permission_group 表 (保護系統管理員帳號)';

-- 清理權限群組 (但保護系統管理員群組 sid=3)
DELETE FROM permission_group 
WHERE sid != 3;  -- 保護系統管理員群組
PRINT '已清理 permission_group 表 (保護系統管理員群組)';

PRINT '清理完成 - 已保護新系統核心設置';
GO

-- ==========================================
-- STEP 2: 遷移權限群組 (使用 MERGE 避免衝突)
-- ==========================================
PRINT '=== 開始遷移權限群組 ===';
...
USING (
    ...
    WHERE sag.type = 'P'  -- 只處理人員類型
      ...
      AND sag.target_sid != 1  -- 排除系統管理員，避免覆蓋現有設置
) AS source
...
USING (
    ...
    WHERE sgr.groups_sid IS NOT NULL
      AND sgr.groups_sid != 3  -- 排除系統管理員群組，避免覆蓋現有設置
