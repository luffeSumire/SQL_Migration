...
UPDATE [Ecocampus].[dbo].[Accounts] SET SchoolId = NULL WHERE SchoolId IS NOT NULL
PRINT '✓ 已清空 Accounts.SchoolId 筆數: ' + CAST(@@ROWCOUNT AS VARCHAR);

-- 清除所有依賴Schools表的資料表（按依賴順序）
DELETE FROM [Ecocampus].[dbo].[CampusSubmissions]            -- 校園申請資料
PRINT ' - 已刪除 CampusSubmissions: ' + CAST(@@ROWCOUNT AS VARCHAR);
DELETE FROM [Ecocampus].[dbo].[SchoolStatistics]             -- 學校統計資料
PRINT ' - 已刪除 SchoolStatistics: ' + CAST(@@ROWCOUNT AS VARCHAR);
DELETE FROM [Ecocampus].[dbo].[SchoolPrincipals]             -- 學校校長資料
PRINT ' - 已刪除 SchoolPrincipals: ' + CAST(@@ROWCOUNT AS VARCHAR);
DELETE FROM [Ecocampus].[dbo].[SchoolEnvironmentalPathStatuses]  -- 學校環境路徑狀態
PRINT ' - 已刪除 SchoolEnvironmentalPathStatuses: ' + CAST(@@ROWCOUNT AS VARCHAR);
DELETE FROM [Ecocampus].[dbo].[SchoolContacts]               -- 學校聯絡人資料
PRINT ' - 已刪除 SchoolContacts: ' + CAST(@@ROWCOUNT AS VARCHAR);
DELETE FROM [Ecocampus].[dbo].[SchoolContents]               -- 學校多語系內容
PRINT ' - 已刪除 SchoolContents: ' + CAST(@@ROWCOUNT AS VARCHAR);
DELETE FROM [Ecocampus].[dbo].[Certifications]               -- 認證資料
PRINT ' - 已刪除 Certifications: ' + CAST(@@ROWCOUNT AS VARCHAR);
DELETE FROM [Ecocampus].[dbo].[Schools]                      -- 學校主表（最後清除）
...
INSERT INTO [Ecocampus].[dbo].[SchoolContents] (
    SchoolId,                            -- 學校ID
...
INSERT INTO [Ecocampus].[dbo].[Certifications]
(
	[SchoolId],                          -- 學校ID
...
INNER JOIN [Ecocampus].[dbo].[Schools] S ON  -- 關聯新系統學校表
...
LEFT JOIN [Ecocampus].[dbo].[Certifications] SC ON  -- 檢查是否已存在相同認證
...
INSERT INTO [Ecocampus].[dbo].[SchoolEnvironmentalPathStatuses]
(
	[SchoolId],                          -- 學校ID
...
INNER JOIN [Ecocampus].[dbo].[Schools] S       -- 關聯新系統學校表
	ON S.SchoolCode = E.SchoolCode
LEFT JOIN #EnviromentMapping EM                             -- 關聯環境路徑對照表
	ON EM.[Text] = E.[PathName]
LEFT JOIN [Ecocampus].[dbo].[SchoolEnvironmentalPathStatuses] SEPS  -- 檢查是否已存在
...
SET IDENTITY_INSERT [Ecocampus].[dbo].[SchoolPrincipals] ON;

-- 將校長資料插入SchoolPrincipals表
INSERT INTO [Ecocampus].[dbo].[SchoolPrincipals]
(
    [Id],                                -- 使用原系統的ID
    [SchoolId],                          -- 學校ID
    [PrincipalName],                     -- 校長姓名
...
INNER JOIN [Ecocampus].[dbo].[Schools] s ON s.SchoolCode = CASE 
    ...
    ELSE cm.code 
END
LEFT JOIN [Ecocampus].[dbo].[SchoolPrincipals] existing_sp ON existing_sp.Id = csp.sid
...
-- 關閉 IDENTITY_INSERT
SET IDENTITY_INSERT [Ecocampus].[dbo].[SchoolPrincipals] OFF;
...
-- 啟用 IDENTITY_INSERT 以插入明確的 Id 值
SET IDENTITY_INSERT [Ecocampus].[dbo].[SchoolStatistics] ON;

-- 將學校統計資料插入SchoolStatistics表
INSERT INTO [Ecocampus].[dbo].[SchoolStatistics]
(
    [Id],                                -- 使用原系統的ID
    [SchoolId],                          -- 學校ID
    [StaffTotal],                        -- 教職員總數
...
INNER JOIN [Ecocampus].[dbo].[Schools] s ON s.SchoolCode = CASE 
    ...
    ELSE cm.code 
END
LEFT JOIN [Ecocampus].[dbo].[SchoolStatistics] existing_ss ON existing_ss.Id = css.sid
...
-- 關閉 IDENTITY_INSERT
SET IDENTITY_INSERT [Ecocampus].[dbo].[SchoolStatistics] OFF;
...
-- 啟用 IDENTITY_INSERT 以插入明確的 Id 值
SET IDENTITY_INSERT [Ecocampus].[dbo].[SchoolContacts] ON;

-- 將學校聯絡人資料插入SchoolContacts表
INSERT INTO [Ecocampus].[dbo].[SchoolContacts]
(
    [Id],                                -- 使用原系統的ID
    [SchoolId],                          -- 學校ID
    [ContactName],                       -- 聯絡人姓名
...
INNER JOIN [Ecocampus].[dbo].[Schools] s ON s.SchoolCode = CASE 
    ...
    ELSE cm.code 
END
LEFT JOIN [Ecocampus].[dbo].[SchoolContacts] existing_sc ON existing_sc.Id = cc.sid
...
-- 關閉 IDENTITY_INSERT
SET IDENTITY_INSERT [Ecocampus].[dbo].[SchoolContacts] OFF;
