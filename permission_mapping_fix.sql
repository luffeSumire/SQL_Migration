...
-- 移除群組存在性檢查，允許所有群組包括不存在於 permission_group 的群組
...
SELECT 
    ...
    ISNULL(pmc.permission_count, 0) as permission_count,
    ISNULL(pmc.allowed_count, 0) as allowed_count
FROM permission_group pg
LEFT JOIN (
    SELECT groupSid, COUNT(*) as permission_count, SUM(CAST(allow AS INT)) as allowed_count
    FROM permission_group_map 
    GROUP BY groupSid
) pmc ON pmc.groupSid = pg.sid
ORDER BY pg.sid
